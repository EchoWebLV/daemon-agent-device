  ...
  async function handleSend() {
    ...
    try {
      ...
      const systemContent = [
        ...
        '',
        '## Tools Available',
        'Built-in (free) tools: swap quotes, token safety/rug checks, token profiles, trending tokens, new pair launches,',
        ...
        ...(enabledServiceIds.length > 0
          ? [
              '',
              `x402 paid tools (enabled by the user — cost USDC per call): ${getEnabledServiceNames(enabledServiceIds).join(', ')}.`,
              'Tool names start with "x402_". Inspect your tool schema for the exact endpoints and descriptions.',
            ]
          : []),
        '',
        'When the user asks "what can you do", "what tools do you have", or anything similar, enumerate EVERY tool you actually',
        'have — built-in AND every x402 tool in your tool schema — by name with a one-line purpose. Do not summarize into',
        'generic buckets; do not omit the x402 tools. The user wants to see the full list, including the paid ones they enabled.',
